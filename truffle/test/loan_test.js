...
contract("Loan", function(accounts) {
    ...
    it("Fails to provide more funds that those who were requested for loan", async function() {
        await expectRevert.unspecified(
            tuiChainLoan.provideFunds(BigInt(2000) * BigInt(10) ** BigInt(18), {
                from: accounts[2]
            })
        );
    });
    ...
    it("Provides funds to the loan", async function() {
        fundsToProvide = 100;
        const initialInvestorFunds = fundsToInt(
        ...
        const finalInvestorFunds = fundsToInt(
            await daiMock.balanceOf(accounts[2])
        );

        assert(
            finalInvestorFunds ==
                initialInvestorFunds - fundsToProvide * (1 + fundingFee)
        );
    ...
    it("Withdraws the provided funds", async function() {
        ...
        await tuiChainLoan.withdrawFunds(
            BigInt(fundsToProvide) * BigInt(10) ** BigInt(18),
            { from: accounts[2] }
        );
        const fundedDai = fundsToInt(await tuiChainLoan.fundedValueAttoDai());
    ...
    it("Provides the remaining funds to the loan", async function() {
        const initialStundentFunds = fundsToInt(
        ...
        const finalStundentFunds = fundsToInt(
            await daiMock.balanceOf(accounts[1])
        );
        const finalInvestorFunds = fundsToInt(
            await daiMock.balanceOf(accounts[2])
        ...
        assert(
            finalInvestorFunds ==
                initialInvestorFunds - providedFunds * (1 + fundingFee)
        );
    });
    ...
    it("Fails to provide funds for a loan not in funding phase", async function() {
        ...
        await expectRevert.unspecified(
            tuiChainLoan.provideFunds(
                BigInt(providedFunds - fundsToProvide) *
                    BigInt(10) ** BigInt(18),
                { from: accounts[2] }
            )
        );
    ...
    it("Makes payment", async function() {
        ...
        const initialStudentDaiAmount = fundsToInt(
            await daiMock.balanceOf(accounts[1])
        );
        ...
        const finalStudentDaiAmount = fundsToInt(
            await daiMock.balanceOf(accounts[1])
        );

        assert(paymentAmount == paidValueDai);
        assert(
            finalStudentDaiAmount ==
                initialStudentDaiAmount - paymentAmount * (1 + paymentFee)
        );
    });
    ...
    it("Fails to reedem tokens when fase is not finalized", async function() {
        ...
        await expectRevert.unspecified(
            tuiChainLoan.redeemTokens(10, { from: accounts[2] })
        );
    });
    ...
    it("Fails to reedem tokens when account as no one to reedem", async function() {
        ...
        assert(phase == Phase.Finalized);
        await expectRevert.unspecified(
            tuiChainLoan.redeemTokens(10, { from: accounts[5] })
        );
    });
    ...
    it("Fails to reedem more tokens than the available ones", async function() {
        ...
        assert(phase == Phase.Finalized);
        await expectRevert.unspecified(
            tuiChainLoan.redeemTokens(2000, { from: accounts[2] })
        );
    });

    it("Reedems tokens", async function() {
        const phase = (await tuiChainLoan.phase()).toNumber();
        const initialDaiBalance = fundsToInt(
            await daiMock.balanceOf(accounts[2])
        );
        ...
        const finalDaiBalance = fundsToInt(
            await daiMock.balanceOf(accounts[2])
        );
        const redemptionValueDaiPerToken = fundsToInt(
            await tuiChainLoan.redemptionValueAttoDaiPerToken()
        );

        assert(
            finalDaiBalance ==
                initialDaiBalance + redemptionValueDaiPerToken * tokensToRedeem
        );
    });
});
