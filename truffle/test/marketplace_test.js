...
contract("Marketplace", function(accounts) {
  ...
  it("Fails to increase the amount of a non-existent sell position", async function() {
    await expectRevert.unspecified(
      tuiChainMarket.increaseSellPositionAmount(tuiChainToken.address, 1, {
        from: accounts[1]
      })
    );
  });
  ...
  it("Fails to decrease the amount of a non-existent sell position", async function() {
    await expectRevert.unspecified(
      tuiChainMarket.decreaseSellPositionAmount(tuiChainToken.address, 1, {
        from: accounts[1]
      })
    );
  });
  ...
  it("Fails to decrease the amount of a sell position to a negative value", async function() {
    await expectRevert.unspecified(
      tuiChainMarket.decreaseSellPositionAmount(tuiChainToken.address, 100, {
        from: accounts[2]
      })
    );
  });
  ...
  it("Buys from existent sell position", async function() {
    ...
    assert(
      finalBuyerDai == initialBuyerDai - tokensToBuy * 60 * (1 + fundingFee)
    );
  });
