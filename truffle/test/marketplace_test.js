...
contract("Marketplace", function (accounts) {
  ...
  it("Fails to update a non-existent sell position price", async function () {
    ...
    await expectRevert.unspecified(
      tuiChainMarket.updateSellPositionPrice(tuiChainToken.address, updatePrice, {from: accounts[1]})
    );
  });

  it("Updates a existent sell position price", async function () {
    const updatePrice = BigInt(60) * (BigInt(10) ** BigInt(18));
    await tuiChainMarket.updateSellPositionPrice(tuiChainToken.address, updatePrice, {from: accounts[2]});
  ...
  it("Fails to increase the amount of a non-existent sell position", async function () {
    await expectRevert.unspecified(
      tuiChainMarket.increaseSellPositionAmount(tuiChainToken.address, 1, {from: accounts[1]})
    );
  });
  ...
  it("Increases the amount of a sell position", async function () {
    ...
    await tuiChainMarket.increaseSellPositionAmount(tuiChainToken.address, tokensToIncrease, {from: accounts[2]});
    const finalTokensBalance = (await tuiChainToken.balanceOf(accounts[2])).toNumber();
  ...
  it("Fails to decrease the amount of a non-existent sell position", async function () {
    await expectRevert.unspecified(
      tuiChainMarket.decreaseSellPositionAmount(tuiChainToken.address, 1, {from: accounts[1]})
    );
  });

  it("Fails to decrease the amount of a sell position to a negative value", async function () {
    await expectRevert.unspecified(
      tuiChainMarket.decreaseSellPositionAmount(tuiChainToken.address, 100, {from: accounts[2]})
    );
  });
  ...
  it("Decreases the amount of a sell position", async function () {
    ...
    await tuiChainMarket.decreaseSellPositionAmount(tuiChainToken.address, tokensToDecrease, {from: accounts[2]});
    const finalTokensBalance = (await tuiChainToken.balanceOf(accounts[2])).toNumber();
  ...
  it("Fails to remove non-existent sell position", async function () {
    await expectRevert.unspecified(
      tuiChainMarket.removeSellPosition(tuiChainToken.address, {from: accounts[1]})
    );
  });

  it("Removes existent sell position", async function () {
    await tuiChainMarket.removeSellPosition(tuiChainToken.address, {from: accounts[2]});
    const finalTokensBalance = (await tuiChainToken.balanceOf(accounts[2])).toNumber();
